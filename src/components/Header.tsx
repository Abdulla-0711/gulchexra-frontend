      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            {navItems.map(item => (
              <Link
                ...
                className={`font-body text-sm font-medium transition-colors hover:text-accent ${
                  location.pathname === item.to ? 'text-accent' : 'text-foreground/70'
                }`}
              >
                {item.label}
              </Link>
          ...
          <div className="flex items-center gap-2">
                    ...
                    {languages.map(l => (
                      <button
                        ...
                        className={`block w-full text-left px-4 py-2 text-sm hover:bg-accent/10 transition-colors ${
                          lang === l.code ? 'text-accent font-semibold' : 'text-foreground/70'
                        }`}
                      >
                        {l.label}
                      </button>
            ...
            <button onClick={toggleTheme} className="p-2 rounded-md text-foreground/70 hover:text-foreground transition-colors">
              {theme === 'light' ? <Moon className="w-4 h-4" /> : <Sun className="w-4 h-4" />}
            </button>

            <Link to="/cart" className="relative p-2 rounded-md text-foreground/70 hover:text-foreground transition-colors">
